import {
    ...
    REQUEST_DATA, REQUEST_DATA_SUCCESS,
    SET_CURRENT_YEAR,
    MODAL_TOGGLE, REQUEST_GEOJSON, RECIEVE_GEOJSON,
    ENABLE_BUILD, DESELECT_ALL,
    SAVE_VIZ, SAVE_VIZ_COMPLETE,
    REQUEST_AVERAGES, RECIEVE_AVERAGES, TOGGLE_AVERAGE,
    START_PLAYING, PAUSE_PLAYING, ENABLE_UNBUILD,
    SET_MAPTYPE
...
export default function visualize(state = initialState, action) {
    switch (action.type) {
        ...
        case TOGGLE_AVERAGE:
            return {
                ...state,
                data: toggleAverage(state.data, state.averagesData[action.spec], action.spec)
            };
        case START_PLAYING:
            return {
                ...state,
