...
const Region = model.Region;
   ...
   */
  router.report('/averages/weights', (req, res) => {
...
let getIndicatorData = async(toGet => {
  return new Promise((resolve, reject) => {
    let responseSet = [];
    try {
      _.forEach(toGet, setup => {
...
let queryIndicatorData = async(_setup => {
  return new Promise((resolve, reject) => {
    let countries = [];
    let regions = [];
    ...
    try {
      ...
      _.each(_setup.countries, cty => {
        ...
        let exists = _.findIndex(dataSet, o => { return o.Location_ID == cty.Country_ID; })
        if (exists != -1) {
          countries.push(cty);
        } else {
      ...
      _.each(_setup.regions, reg => {
        // make sure there is data in response object
        let exists = _.findIndex(dataSet, o => { return o.Location_ID == reg.Region_ID; })
        if (exists != -1) {
          regions.push(reg);
        } else {
      ...
      meta = await(model.Indicator.findAll({
        attributes: [
          'Indicator_Name', 'Direct_Indicator_Source', 'Original_Indicator_Source',
          'Indicator_URL', 'Indicator_Data_URL', 'Units', 'Indicator_Definition'
        ],
        where: {
          Indicator_ID: indIds
        }
...
function dataSetFormation(ctyArr, regArr) {
  ...
  return aggregateSet.concat(_.map(regArr, reg => {
    return {
...
let nestedAvailibiltyCheck = async(toGet => {
  return new Promise((resolve, reject) => {
    var responseSet = [];
    try {
      _.forEach(toGet, setup => {
...
let performAvailibiltyCheck = async((_setup) => {
  return new Promise((resolve, reject) => {
    let indIds = _setup.indicators.map(ind => {
      return ind.id;
    });
    ...
    try {
      if (ctyIds.length != 0) {
        ctyAvil = await(model.sequelize.query(`
                    ...
                    ORDER BY "Date"
                 `, { replacements: { 'ctys': ctyIds, 'inds': indIds }, type: model.sequelize.QueryTypes.SELECT }));
      }
      if (regIds.length != 0) {
        regAvil = await(model.sequelize.query(`
                    ...
                    ORDER BY "Year"
                 `, { replacements: { 'regs': regIds, 'inds': indIds }, type: model.sequelize.QueryTypes.SELECT }));
      }
      ...
      _.each(ctyAvil, (obj, i) => {
        ...
        _.each(ctyIds, id => {
          if (obj.countries_without.indexOf(id) != -1) {
            let i = _.findIndex(_setup.countries, c => { return c.Country_ID == id });
            parsed[obj.Date].locations_no_data.push(" " + _setup.countries[i].Name);
          }
        });

        parsed[obj.Date].missingpoints = parsed[obj.Date].locations_no_data.length;
      ...
      _.each(regAvil, (obj, i) => {
        ...
        _.each(regIds, id => {
          if (obj.regions_without.indexOf(id) != -1) {
            let i = _.findIndex(_setup.regions, r => { return r.Region_ID == id });
            parsed[obj.Year].locations_no_data.push(" " + _setup.regions[i].Name);
          }
        });

        parsed[obj.Year].missingpoints = parsed[obj.Year].locations_no_data.length;
        // total data points addition
        parsed['all'].datapoints += parseInt(obj.data_points, 10);
 ...
 */
var getAverageWeights = async(countries => {
  return new Promise((resolve, reject) => {
...
function getPopulationWeight(countries) {
  return new Promise((resolve, reject) => {
    Data.findAll({
      attributes: ['Country_ID', 'Date', model.sequelize.literal('"Value" / sum("Value") over (PARTITION BY "Date") as perc')],
      where: {
        Indicator_ID: 165,
